...
━━━━━━━━━━━━━━━━━━━━
{EMOJI['target']} Только <b>Melbet</b>
...
def deposit_start(msg):
    if not is_bot_active() and msg.from_user.id not in get_admins() and msg.from_user.id != MAIN_ADMIN:
        ...
        return
    temp_data[msg.chat.id] = {"platform": "Melbet"}
    send_msg(msg.chat.id, f"{EMOJI['info']} <b>Введите ваш ID Melbet:</b>", reply_markup=back_menu())
...
def get_account_id(msg):
    if not msg.text or msg.text.startswith('/start') or msg.text == "🔙 Назад":
        start(msg)
        return
    temp_data.setdefault(msg.chat.id, {})["account_id"] = f"Melbet | {msg.text.strip()}"
    send_msg(msg.chat.id,
...
def get_amount(msg):
    ...
    if qr:
        ...
        send_msg(user_id, f"{EMOJI['qr']} QR пока не загружен админом.")

    text = f"""{EMOJI['sparkles']} <b>Оплата Melbet</b>

━━━━━━━━━━━━━━━━━━━━
🆔 Счёт: <code>{safe_html(account_id)}</code>
{EMOJI['money']} К оплате: <b>{final:.2f} сом</b>

⚠️ Переведите <u>точную сумму</u> с копейками
━━━━━━━━━━━━━━━━━━━━

{EMOJI['clock']} <b>5 минут</b> — пришлите скриншот чека"""
...
def get_check_photo(msg):
    ...
    caption = (f"{EMOJI['lightning']} <b>ЗАЯВКА MELBET #{dep_id}</b>\n\n"
               f"👤 Юзер: {user_id}\n"
...
def withdraw_get_elqr(msg):
    ...
    send_msg(msg.chat.id, f"{EMOJI['info']} <b>Отправьте ваш ID Melbet:</b>", reply_markup=back_menu())
...
def withdraw_get_id_text(msg):
    ...
    if not msg.text.strip():
        ...
        bot.register_next_step_handler(msg, withdraw_get_id_text)
        return
    temp_data.setdefault(msg.chat.id, {})["id_photo"] = f"Melbet | {msg.text.strip()}"

    img = get_withdraw_image()
    if img:
        send_media_bulletproof(msg.chat.id, img)

    instruction = f"""{EMOJI['info']} <b>Инструкция по выводу Melbet</b>

1. Настройки
2. Вывести со счёта
3. <b>MOBCASH</b>
4. Укажите сумму
5. Касса: <b>Касса Бишкек - LMWPAY</b>
6. Подтвердить
7. Получить код
...
def withdraw_get_code(msg):
    ...
    caption = (f"{EMOJI['withdraw']} <b>ВЫВОД MELBET #{w_id}</b>\n\n"
               f"👤 Юзер: {user_id}\n"
        ...
        if row:
            ...
            try:
                bot.send_message(user_id,
                    f"""{EMOJI['check']} <b>Melbet пополнен!</b>
        ...
        if row:
            ...
            try:
                bot.send_message(row[0],
                    f"""{EMOJI['check']} <b>Вывод Melbet выполнен</b>
...
if __name__ == "__main__":
    ...
    logger.info("Melbet Pay started")
    while True:
        try:
