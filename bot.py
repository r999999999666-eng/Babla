...
def save_qr(file_id):
    with get_db() as conn:
        ...
        conn.commit()
    logger.info(f"QR сохранён: {file_id}")
...
def save_withdraw_image(file_id):
    with get_db() as conn:
        ...
        conn.execute('INSERT INTO withdraw_images (file_id, date) VALUES (?, ?)',
                     (file_id, datetime.now().strftime("%d.%m.%Y %H:%M")))
        conn.commit()
    logger.info(f"Картинка инструкции сохранена: {file_id}")
...
━━━━━━━━━━━━━━━━━━━━
{EMOJI['money']} <b>Быстрые пополнения и выводы</b>
...
def get_account_id(msg):
    if not msg.text or msg.text.startswith('/start') or msg.text == "🔙 Назад":
        start(msg)
        return
    temp_data.setdefault(msg.chat.id, {})["account_id"] = f"1xBet | {msg.text.strip()}"
    send_msg(msg.chat.id, f"{EMOJI['money']} <b>Введите сумму</b>\n\nОт <b>15</b> до <b>500 000</b> сом",
             reply_markup=back_menu())
...
def get_amount(msg):
    ...
    try:
        ...
        bot.register_next_step_handler(msg, get_amount)
        return
    if not (15 <= base <= 500000):
        send_msg(msg.chat.id, f"{EMOJI['cross']} Сумма должна быть от 15 до 500 000 сом!", reply_markup=back_menu())
        bot.register_next_step_handler(msg, get_amount)
        return
    ...
    text = f"""{EMOJI['sparkles']} <b>Оплата</b>

━━━━━━━━━━━━━━━━━━━━
🆔 Счёт: <code>{safe_html(account_id)}</code>
💰 К оплате: <b>{final:.2f} сом</b>

⚠️ Переведите <u>точную сумму</u> с копейками
━━━━━━━━━━━━━━━━━━━━

{EMOJI['clock']} У вас есть <b>5 минут</b>
...
{EMOJI['clock']} Ожидайте подтверждения оператора""",
...
def withdraw_get_elqr(msg):
    ...
    send_msg(msg.chat.id, f"{EMOJI['info']} <b>Отправьте ваш ID 1xBet:</b>", reply_markup=back_menu())
...
def withdraw_get_id_text(msg):
    ...
    if not msg.text.strip():
        ...
        bot.register_next_step_handler(msg, withdraw_get_id_text)
        return
    temp_data.setdefault(msg.chat.id, {})["id_photo"] = f"1xBet | {msg.text.strip()}"

    # Отправляем картинку инструкции (если есть)
    img = get_withdraw_image()
    if img:
        send_media_bulletproof(msg.chat.id, img)

    instruction = f"""📍 <b>Инструкция по выводу</b>

1. Настройки
2. Вывести со счёта
3. <b>MOBCASH</b>
4. Укажите сумму
5. Выберите кассу: <b>Касса Ош - TezPay</b>
6. Подтвердить
7. Получить код
...
def withdraw_get_code(msg):
    ...
    caption = (f"{EMOJI['withdraw']} <b>ЗАЯВКА НА ВЫВОД #{w_id}</b>\n\n"
               f"👤 Юзер: {user_id}\n"
               f"🆔 Счёт: <code>{safe_html(id_photo)}</code>\n"
...
def remove_admin_btn(msg):
    if msg.from_user.id != MAIN_ADMIN:
        send_msg(msg.chat.id, f"{EMOJI['cross']} Только главный админ может удалять.")
        return
    send_msg(msg.chat.id, f"{EMOJI['info']} Введите ID администратора для удаления:", reply_markup=back_menu())
...
def process_remove_admin(msg):
    ...
    try:
        ...
        if admin_id == MAIN_ADMIN:
            send_msg(msg.chat.id, f"{EMOJI['cross']} Нельзя удалить главного администратора!", reply_markup=admin_menu())
            return
        remove_admin(admin_id)
        send_msg(msg.chat.id, f"{EMOJI['check']} Администратор <code>{admin_id}</code> удалён!", reply_markup=admin_menu())
    except:
        send_msg(msg.chat.id, f"{EMOJI['cross']} Введите корректный числовой ID!", reply_markup=admin_menu())
...
def change_withdraw_image(msg):
    send_msg(msg.chat.id, f"{EMOJI['qr']} Отправьте картинку инструкции по выводу (скрин MobCash):", reply_markup=back_menu())
...
def save_new_withdraw_image(msg):
    ...
    if file_id:
        ...
        send_msg(msg.chat.id, f"{EMOJI['check']} Картинка инструкции сохранена!", reply_markup=admin_menu())
    else:
        send_msg(msg.chat.id, f"{EMOJI['cross']} Отправьте изображение!", reply_markup=back_menu())
        bot.register_next_step_handler(msg, save_new_withdraw_image)
...
{EMOJI['money']} Общий объём: <b>{s['total']:.2f} сом</b>
━━━━━━━━━━━━━━━━━━━━""")
...
def broadcast_start(msg):
    send_msg(msg.chat.id, f"{EMOJI['broadcast']} <b>Отправьте сообщение для рассылки</b> (текст или фото):",
             reply_markup=back_menu())
...
💰 Сумма: <b>{amount:.2f} сом</b>
🆔 Счёт: <code>{safe_html(account_id)}</code>
⏱️ Время: <b>{elapsed} сек</b>
━━━━━━━━━━━━━━━━━━━━

Спасибо за использование <b>{BOT_NAME}</b>!""")
            except:
                pass
            try:
        ...
        if row:
            ...
            try:
                bot.send_message(row[0],
                    f"""{EMOJI['check']} <b>Успешный вывод средств</b>

━━━━━━━━━━━━━━━━━━━━
🆔 Счёт: <code>{safe_html(row[1])}</code>
{EMOJI['money']} Статус: <b>Выполнено</b>
━━━━━━━━━━━━━━━━━━━━

Спасибо, что пользуетесь <b>{BOT_NAME}</b>!""")
            except:
                pass
        try:
            bot.edit_message_caption(
