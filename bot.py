...
# Канал для обязательной подписки: @username или -100...
CHANNEL_USERNAME = os.environ.get("CHANNEL_USERNAME", "").strip()  # например @DiamondPAY_channel
CHANNEL_LINK = os.environ.get("CHANNEL_LINK", "").strip()  # https://t.me/...
...
def channel_url():
    ...
    if CHANNEL_USERNAME:
        ...
        if u.startswith("-100") or u.lstrip("-").isdigit():
            return None  # только id — ссылку задайте CHANNEL_LINK
        return f"https://t.me/{u}"
    return None


def is_subscribed(user_id):
    """Проверка подписки. Если канал не задан — пропускаем."""
    ...
    try:
        ...
        # если бот не админ канала — не блокируем всех
        return True


def require_subscribe(msg_or_chat):
    """Показать просьбу подписаться. True = уже подписан."""
...
def main_menu(user_id):
    """Главное меню в стиле TezSal (ReplyKeyboard)."""
...
def _find_app_html():
    """Ищем app.html на Render / локально."""
    candidates = [
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "static", "app.html"),
        os.path.join(os.getcwd(), "static", "app.html"),
        os.path.join(os.path.dirname(os.path.abspath(__file__)), "app.html"),
        os.path.join(os.getcwd(), "app.html"),
    ]
...
def mini_app():
    ...
    if not path:
        return (
            "<h3>app.html не найден</h3>"
            "<p>Загрузите файл в папку <b>static/app.html</b> рядом с bot.py</p>",
            404,
        )
