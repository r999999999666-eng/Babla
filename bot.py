...
def tpl_status_line() -> str:
    return f"✅ Актуально на: {datetime.now().strftime('%d.%m.%Y %H:%M')}"
...
🌐 Букмекеры: <b>1xBet</b> · <b>Melbet</b>
💵 Мин. пополнение: <b>{MIN_DEPOSIT}</b> KGS
...
def referral_info(msg):
    ...
    text = (
        ...
        f"✅ Пополнений друзей: <b>{dep_count}</b>\n"
        f"💳 Сумма пополнений друзей: <b>{dep_sum:.2f} KGS</b>\n"
        f"🎁 Доступный баланс: <b>{bal:.2f} KGS</b>\n\n"
    ...
    if bal >= MIN_REF_WITHDRAW:
        kb.add(types.InlineKeyboardButton(
            "Пополнить 1xBet с реф. баланса",
            callback_data="ref_to_1x",
        ))
        kb.add(types.InlineKeyboardButton(
            "Пополнить Melbet с реф. баланса",
            callback_data="ref_to_melbet",
        ))
        kb.add(types.InlineKeyboardButton(
            "Вывести реф. баланс",
            callback_data="ref_out",
        ))
    kb.add(types.InlineKeyboardButton("Главное меню", callback_data="ref_home"))
...
def ref_out_id(msg):
    ...
    if bal < MIN_REF_WITHDRAW:
        send_msg(uid, f"❌ Недостаточно средств (мин. {MIN_REF_WITHDRAW} KGS)", reply_markup=main_menu(uid))
        return
    ...
    markup.add(
        types.InlineKeyboardButton("✅ Зачислено", callback_data=f"refw_ok_{wid}"),
        types.InlineKeyboardButton("❌ Отклонить", callback_data=f"refw_no_{wid}"),
    )
